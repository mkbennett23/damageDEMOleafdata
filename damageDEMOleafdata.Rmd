...
emmeans(insectherbModeloverdisp, pairwise~as.factor(Species)*as.factor(Age_class), adjust="tukey", type = "response") #add type = reponse for poisson distribution, from this website https://cran.r-project.org/web/packages/emmeans/vignettes/sophisticated.html
...
emmeans(insectherbModeloverdisp2, pairwise~as.factor(Species)*as.factor(Age_class), adjust="tukey", type = "response") #add type = reponse for poisson distribution, from this website https://cran.r-project.org/web/packages/emmeans/vignettes/sophisticated.html

blah <- glht(insectherbModeloverdisp2, linfct = mcp(period = "Tukey"))
...
emmeans(insectherbModeloverdisp3, pairwise~as.factor(Species)*as.factor(Age_class), adjust="tukey", type = "response") #add type = reponse for poisson distribution, from this website https://cran.r-project.org/web/packages/emmeans/vignettes/sophisticated.html)
...
emmeans(insectherbModeloverdisp4, pairwise~as.factor(Species)*as.factor(Age_class), adjust="tukey", type = "response") #add type = reponse for poisson distribution, from this website https://cran.r-project.org/web/packages/emmeans/vignettes/sophisticated.html)
```
...
ggplot(data=barGraphStats(data=damageOct, variable="Percent_total_herbivory", byFactorNames=c("Species", "Age_class")), aes(x=Species, y=mean, fill=Age_class)) +
  ...
  xlab(element_blank())+
  expand_limits(y=4) +
  annotate("text", x= 0.77, y = 3.5, label= "b", size = 6)+
  annotate("text", x= 1.23, y = 3.7, label= "b", size = 6)+
  annotate("text", x= 1.77, y = 3.5, label= "b", size = 6)+
  annotate("text", x= 2.23, y = 2.5, label= "b", size = 6)+
  annotate("text", x= 2.77, y = 5.2, label= "ab", size = 6)+
  annotate("text", x= 3.23, y = 5.1, label= "a", size = 6)+
...
**herbivory model 1**
```{r}
ggplot(data=barGraphStats(data=damageJuly, variable="Percent_total_herbivory", byFactorNames=c("Species", "Age_class")), aes(x=Species, y=mean, fill=Age_class)) +
  geom_bar(stat='identity', position=position_dodge()) +
  geom_errorbar(aes(ymin=mean-se, ymax=mean+se), width=0.2, position=position_dodge(0.9))+
  theme(legend.title=element_blank())+
  theme_bw()+
  ...
  theme(legend.title=element_blank(),legend.text=element_text(size=16), axis.text.x=element_text(size=16, color = "black"), axis.title.y=element_text(size=18), axis.text.y=element_text(size=16, color= "black"), panel.grid.major=element_blank(), panel.grid.minor=element_blank())+
  expand_limits(y=9) +
  annotate("text", x= 0.7, y = 2.6, label= "c", size = 6)+
  annotate("text", x= 1.0, y = 3.7, label= "a", size = 6)+
  annotate("text", x= 1.3, y = 3.75, label= "a", size = 6)+
  annotate("text", x= 1.7, y = 1, label= "c", size = 6)+
  annotate("text", x= 2.0, y = 2.1, label= "b", size = 6)+
  annotate("text", x= 2.3, y = 1.9, label= "b", size = 6)+
  annotate("text", x= 2.7, y = 0.9, label= "c", size = 6)+
  annotate("text", x= 3.0, y = 1.7, label= "abc", size = 6)+
  annotate("text", x= 3.3, y = 3.1, label= "a", size = 6)
...
microbial model 2 graph
```{r}

ggplot(data=barGraphStats(data=damageOct, variable="Percent_microbial", byFactorNames=c("Species", "Age_class")), aes(x=Species, y=mean, fill=Age_class)) +
  geom_bar(stat='identity', position=position_dodge()) +
  geom_errorbar(aes(ymin=mean-se, ymax=mean+se), width=0.2, position=position_dodge(0.9))+
  theme_bw()+
  scale_fill_manual(values=c("darkolivegreen3", "darkolivegreen4"))+
  ...
  xlab(element_blank())+
  annotate("text", x= 0.77, y = 5, label= "b", size = 6)+
  annotate("text", x= 1.23, y = 5, label= "b", size = 6)+
  annotate("text", x= 1.77, y = 11.2, label= "a", size = 6)+
  annotate("text", x= 2.23, y = 14, label= "a", size = 6)+
  annotate("text", x= 2.77, y = 14, label= "ab", size = 6)+
  annotate("text", x= 3.23, y = 13, label= "a", size = 6)+
  theme(legend.title=element_blank(),legend.text=element_text(size=16), axis.text.x=element_text(size=16, color = "black"), axis.title.y=element_text(size=18), axis.text.y=element_text(size=16, color= "black"), panel.grid.major=element_blank(), panel.grid.minor=element_blank())+
  expand_limits(y=17)
...
```{r}
ggplot(data=barGraphStats(data=damageJuly, variable="Percent_microbial", byFactorNames=c("Species", "Age_class")), aes(x=Species, y=mean, fill=Age_class)) +
  geom_bar(stat='identity', position=position_dodge()) +
  geom_errorbar(aes(ymin=mean-se, ymax=mean+se), width=0.2, position=position_dodge(0.9))+
  theme(legend.title=element_blank())+
  theme_bw()+
  ...
  theme(legend.title=element_blank(),legend.text=element_text(size=16), axis.text.x=element_text(size=16, color = "black"), axis.title.y=element_text(size=18), axis.text.y=element_text(size=16, color= "black"), panel.grid.major=element_blank(), panel.grid.minor=element_blank())+
  expand_limits(y=17) +
  annotate("text", x= 0.7, y = 2.4, label= "bc", size = 6)+
  annotate("text", x= 1.0, y = 1.5, label= "d", size = 6)+
  annotate("text", x= 1.3, y = 1.6, label= "d", size = 6)+
  annotate("text", x= 1.7, y = 2.4, label= "c", size = 6)+
  annotate("text", x= 2.0, y =5, label= "a", size = 6)+
  annotate("text", x= 2.3, y = 5, label= "a", size = 6)+
  annotate("text", x= 2.7, y = 5, label= "ab", size = 6)+
  annotate("text", x= 3.0, y = 6, label= "a", size = 6)+
  annotate("text", x= 3.3, y = 5.7, label= "a", size = 6)

```

######Regressions###

*Herbivory by Plot Age and life stage (age_class)* One model per species, per sample period

```{r}

damagePlots$Percent_total_herbivory <- as.integer(as.numeric(damagePlots$Percent_total_herbivory))

herbivoryAgeBeech_lm <-glmmTMB(Percent_total_herbivory~ Age_class*plot_age+(1|Plot/Tree_num), data = subset(damagePlots, Sample_period == 1 & Species == "Beech"), family = genpois)


##model checking##
model_check5 <- simulateResiduals(herbivoryAgeBeech_lm)
plot(model_check5) 

#Amy also ran...#
testZeroInflation(herbivoryAgeBeech_lm)
testUniformity(herbivoryAgeBeech_lm) 
testOutliers(herbivoryAgeBeech_lm) 

drop1(herbivoryAgeBeech_lm, test = 'Chisq')


Anova(herbivoryAgeBeech_lm)


plot(allEffects(herbivoryAgeBeech_lm))
```

Single term deletions

Model:
Percent_total_herbivory ~ Age_class * plot_age + (1 | Plot/Tree_num)
                   Df    AIC    LRT Pr(>Chi)  
<none>                4698.9                  
Age_class:plot_age  2 4699.9 5.0292  0.08089 .
---
Signif. codes:  0 ‘***’ 0.001 ‘**’ 0.01 ‘*’ 0.05 ‘.’ 0.1 ‘ ’ 1

```{r}
herbivoryAgeBeech2_lm <-glmmTMB(Percent_total_herbivory~ Age_class*plot_age+(1|Plot/Tree_num), data = subset(damagePlots, Sample_period == 2 & Species == "Beech"), family = genpois)


##model checking##
model_check6 <- simulateResiduals(herbivoryAgeBeech2_lm)
plot(model_check6) 

#Amy also ran...#
testZeroInflation(herbivoryAgeBeech2_lm)
testUniformity(herbivoryAgeBeech2_lm) 
testOutliers(herbivoryAgeBeech2_lm) 

drop1(herbivoryAgeBeech2_lm, test = 'Chisq')


Anova(herbivoryAgeBeech2_lm)


#emmeans(herbivoryAgeBeech2_lm, pairwise~as.factor(plot_age)*as.factor(Age_class), adjust="tukey", type = "response")

```
	DHARMa zero-inflation test via comparison to expected zeros with simulation under H0 = fitted model

data:  simulationOutput
ratioObsSim = 1.0073, p-value = 0.768
...
D = 0.017521, p-value = 0.8896
alternative hypothesis: two-sided


	DHARMa outlier test based on exact binomial test with approximate expectations

data:  herbivoryAgeBeech2_lm
outliers at both margin(s) = 5, observations = 1096, p-value = 0.3029
alternative hypothesis: true probability of success is not equal to 0.007968127
95 percent confidence interval:
 0.001482892 0.010613933
sample estimates:
...
Percent_total_herbivory ~ Age_class * plot_age + (1 | Plot/Tree_num)
                   Df    AIC    LRT Pr(>Chi)
<none>                4093.7                
Age_class:plot_age  1 4092.9 1.1885   0.2756
Analysis of Deviance Table (Type II Wald chisquare tests)

Response: Percent_total_herbivory
                    Chisq Df Pr(>Chisq)
Age_class          0.5092  1     0.4755
plot_age           2.1744  1     0.1403
Age_class:plot_age 1.1938  1     0.2746

```{r}
herbivoryAgeSG_lm <- glmmTMB(Percent_total_herbivory~ Age_class*plot_age+(1|Plot/Tree_num), data = subset(damagePlots, Sample_period == 1 & Species == "Sweet gum"), family = genpois)

##model checking##
model_check7 <- simulateResiduals(herbivoryAgeSG_lm)
plot(model_check7) 

#Amy also ran...#
testZeroInflation(herbivoryAgeSG_lm)
testUniformity(herbivoryAgeSG_lm) 
testOutliers(herbivoryAgeSG_lm) 

drop1(herbivoryAgeSG_lm, test = 'Chisq')


Anova(herbivoryAgeSG_lm)

plot(allEffects(herbivoryAgeSG_lm))

#emmeans(herbivoryAgeSG_lm, pairwise~as.factor(plot_age)*as.factor(Age_class), adjust="tukey", type = "response")
...
Response: Percent_total_herbivory
                     Chisq Df Pr(>Chisq)    
Age_class          51.8838  2  5.415e-12 ***
plot_age            7.0069  1   0.008119 ** 
Age_class:plot_age  2.5593  2   0.278131    
---
Signif. codes:  0 ‘***’ 0.001 ‘**’ 0.01 ‘*’ 0.05 ‘.’ 0.1 ‘ ’ 1

```{r}
herbivoryAgeSG2_lm <- glmmTMB(Percent_total_herbivory~ Age_class*plot_age+(1|Plot/Tree_num), data = subset(damagePlots, Sample_period == 2 & Species == "Sweet gum"), family = genpois)

##model checking##
model_check8 <- simulateResiduals(herbivoryAgeSG2_lm)
plot(model_check8) 

#Amy also ran...#
testZeroInflation(herbivoryAgeSG2_lm)
testUniformity(herbivoryAgeSG2_lm) 
testOutliers(herbivoryAgeSG2_lm) 

drop1(herbivoryAgeSG2_lm, test = 'Chisq')


Anova(herbivoryAgeSG2_lm)

plot(allEffects(herbivoryAgeSG2_lm))
#emmeans(herbivoryAgeSG2_lm, pairwise~as.factor(plot_age)*as.factor(Age_class), adjust="tukey", type = "response")
```
NA/NaN function evaluation
	DHARMa zero-inflation test via comparison to expected zeros with simulation under H0 = fitted model

data:  simulationOutput
ratioObsSim = 1.0241, p-value = 0.784
alternative hypothesis: two.sided
...
data:  simulationOutput$scaledResiduals
D = 0.025166, p-value = 0.4292
alternative hypothesis: two-sided


	DHARMa outlier test based on exact binomial test with approximate expectations

data:  herbivoryAgeSG2_lm
outliers at both margin(s) = 8, observations = 1207, p-value = 0.7457
alternative hypothesis: true probability of success is not equal to 0.007968127
95 percent confidence interval:
 0.002865716 0.013017836
sample estimates:
frequency of outliers (expected: 0.00796812749003984 ) 
                                           0.006628003 
...
Percent_total_herbivory ~ Age_class * plot_age + (1 | Plot/Tree_num)
                   Df    AIC     LRT Pr(>Chi)
<none>                4065.0                 
Age_class:plot_age  1 4063.2 0.19872   0.6558
Analysis of Deviance Table (Type II Wald chisquare tests)

Response: Percent_total_herbivory
                    Chisq Df Pr(>Chisq)  
Age_class          3.1502  1    0.07592 .
plot_age           5.1938  1    0.02267 *
Age_class:plot_age 0.1989  1    0.65561  
---
Signif. codes:  0 ‘***’ 0.001 ‘**’ 0.01 ‘*’ 0.05 ‘.’ 0.1 ‘ ’ 1

```{r}
herbivoryAgeTP_lm <- glmmTMB(Percent_total_herbivory~ Age_class*plot_age+(1|Plot/Tree_num), data = subset(damagePlots, Sample_period == 1 & Species == "Tulip poplar"), family = genpois)

##model checking##
model_check9 <- simulateResiduals(herbivoryAgeTP_lm)
plot(model_check9) 

#Amy also ran...#
testZeroInflation(herbivoryAgeTP_lm)
testUniformity(herbivoryAgeTP_lm) 
testOutliers(herbivoryAgeTP_lm) 

drop1(herbivoryAgeTP_lm, test = 'Chisq')


Anova(herbivoryAgeTP_lm)


#emmeans(herbivoryAgeTP_lm, pairwise~as.factor(plot_age)*as.factor(Age_class), adjust="tukey", type = "response")
```
NA/NaN function evaluation
	DHARMa zero-inflation test via comparison to expected zeros with simulation under H0 = fitted model

data:  simulationOutput
ratioObsSim = 1.0274, p-value = 0.824
alternative hypothesis: two.sided
...
data:  simulationOutput$scaledResiduals
D = 0.055318, p-value = 0.05116
alternative hypothesis: two-sided


	DHARMa outlier test based on exact binomial test with approximate expectations

data:  herbivoryAgeTP_lm
outliers at both margin(s) = 7, observations = 599, p-value = 0.2529
alternative hypothesis: true probability of success is not equal to 0.007968127
95 percent confidence interval:
 0.00471099 0.02392896
sample estimates:
frequency of outliers (expected: 0.00796812749003984 ) 
                                            0.01168614 

NA/NaN function evaluationSingle term deletions

Model:
Percent_total_herbivory ~ Age_class * plot_age + (1 | Plot/Tree_num)
                   Df    AIC    LRT Pr(>Chi)
<none>                2024.4                
Age_class:plot_age  2 2024.7 4.2983   0.1166
Analysis of Deviance Table (Type II Wald chisquare tests)

Response: Percent_total_herbivory
                     Chisq Df Pr(>Chisq)    
Age_class          28.5919  2  6.185e-07 ***
plot_age            0.1600  1     0.6891    
Age_class:plot_age  2.3282  2     0.3122    
---
Signif. codes:  0 ‘***’ 0.001 ‘**’ 0.01 ‘*’ 0.05 ‘.’ 0.1 ‘ ’ 1
...
```{r}
herbivoryAgeTP2_lm <- glmmTMB(Percent_total_herbivory~ Age_class*plot_age+(1|Plot/Tree_num), data = subset(damagePlots, Sample_period == 2 & Species == "Tulip poplar"), family = genpois)

##model checking##
model_check10 <- simulateResiduals(herbivoryAgeTP2_lm)
plot(model_check10) 

#Amy also ran...#
testZeroInflation(herbivoryAgeTP2_lm)
testUniformity(herbivoryAgeTP2_lm) 
testOutliers(herbivoryAgeTP2_lm) 

drop1(herbivoryAgeTP2_lm, test = 'Chisq')


Anova(herbivoryAgeTP2_lm)


#emmeans(herbivoryAgeTP2_lm, pairwise~as.factor(plot_age)*as.factor(Age_class), adjust="tukey", type = "response")
```
NA/NaN function evaluation
	DHARMa zero-inflation test via comparison to expected zeros with simulation under H0 = fitted model

data:  simulationOutput
ratioObsSim = 0.98262, p-value = 1
alternative hypothesis: two.sided
...
data:  simulationOutput$scaledResiduals
D = 0.040629, p-value = 0.3936
alternative hypothesis: two-sided


	DHARMa bootstrapped outlier test

data:  herbivoryAgeTP2_lm
outliers at both margin(s) = 5, observations = 490, p-value = 0.26
alternative hypothesis: two.sided
 percent confidence interval:
 0.00000000 0.01127551
sample estimates:
outlier frequency (expected: 0.00551020408163265 ) 
                                        0.01020408 
...
Percent_total_herbivory ~ Age_class * plot_age + (1 | Plot/Tree_num)
                   Df    AIC    LRT Pr(>Chi)
<none>                2434.4                
Age_class:plot_age  1 2432.7 0.2838   0.5942
Analysis of Deviance Table (Type II Wald chisquare tests)

Response: Percent_total_herbivory
                    Chisq Df Pr(>Chisq)
Age_class          0.1397  1     0.7085
plot_age           0.4957  1     0.4814
Age_class:plot_age 0.3151  1     0.5746


*Microbial Damage by Plot Age and life stage (age_class)* One model per species, per sample period

```{r}
microbialAgeBeech_lm <- glmmTMB(Percent_microbial~ Age_class*plot_age+(1|Plot/Tree_num), data = subset(damagePlots, Sample_period == 1 & Species == "Beech"), family = genpois)

##model checking##
model_check11 <- simulateResiduals(microbialAgeBeech_lm)
plot(model_check11) 

#Amy also ran...#
testZeroInflation(microbialAgeBeech_lm)
testUniformity(microbialAgeBeech_lm) 
testOutliers(microbialAgeBeech_lm) 

drop1(microbialAgeBeech_lm, test = 'Chisq')


Anova(microbialAgeBeech_lm)


#emmeans(microbialAgeBeech_lm, pairwise~as.factor(plot_age)*as.factor(Age_class), adjust="tukey", type = "response")
```
non-integer counts in a genpois model
	DHARMa zero-inflation test via comparison to expected zeros with simulation under H0 = fitted model

data:  simulationOutput
ratioObsSim = 0.68314, p-value < 2.2e-16
alternative hypothesis: two.sided
...
data:  microbialAgeBeech_lm
outliers at both margin(s) = 17, observations = 1273, p-value = 0.03885
alternative hypothesis: true probability of success is not equal to 0.007968127
95 percent confidence interval:
 0.007798085 0.021295642
sample estimates:
frequency of outliers (expected: 0.00796812749003984 ) 
                                            0.01335428 

non-integer counts in a genpois modelSingle term deletions

Model:
Percent_microbial ~ Age_class * plot_age + (1 | Plot/Tree_num)
                   Df    AIC    LRT Pr(>Chi)
<none>                2819.0                
Age_class:plot_age  2 2816.4 1.3903    0.499
Analysis of Deviance Table (Type II Wald chisquare tests)

Response: Percent_microbial
                     Chisq Df Pr(>Chisq)    
Age_class          22.5707  2  1.256e-05 ***
plot_age            1.4835  1     0.2232    
Age_class:plot_age  1.3812  2     0.5013    
---
Signif. codes:  0 ‘***’ 0.001 ‘**’ 0.01 ‘*’ 0.05 ‘.’ 0.1 ‘ ’ 1

```{r}
microbialAgeBeech2_lm <- glmmTMB(Percent_microbial~ Age_class*plot_age+(1|Plot/Tree_num), data = subset(damagePlots, Sample_period == 2 & Species == "Beech"), family = genpois)

##model checking##
model_check12 <- simulateResiduals(microbialAgeBeech2_lm)
plot(model_check12) 

#Amy also ran...#
testZeroInflation(microbialAgeBeech2_lm)
testUniformity(microbialAgeBeech2_lm) 
testOutliers(microbialAgeBeech2_lm) 

drop1(microbialAgeBeech2_lm, test = 'Chisq')


Anova(microbialAgeBeech2_lm)


#emmeans(microbialAgeBeech2_lm, pairwise~as.factor(plot_age)*as.factor(Age_class), adjust="tukey", type = "response")
```
non-integer counts in a genpois modelNA/NaN function evaluation
	DHARMa zero-inflation test via comparison to expected zeros with simulation under H0 = fitted model

data:  simulationOutput
ratioObsSim = 0.68952, p-value = 0.008
alternative hypothesis: two.sided
...
data:  simulationOutput$scaledResiduals
D = 0.032485, p-value = 0.1977
alternative hypothesis: two-sided


	DHARMa outlier test based on exact binomial test with approximate expectations

data:  microbialAgeBeech2_lm
outliers at both margin(s) = 12, observations = 1096, p-value = 0.235
alternative hypothesis: true probability of success is not equal to 0.007968127
95 percent confidence interval:
 0.00566990 0.01904736
sample estimates:
frequency of outliers (expected: 0.00796812749003984 ) 
                                            0.01094891 

non-integer counts in a genpois modelNA/NaN function evaluationSingle term deletions

Model:
Percent_microbial ~ Age_class * plot_age + (1 | Plot/Tree_num)
                   Df    AIC    LRT Pr(>Chi)
<none>                4961.3                
Age_class:plot_age  1 4959.7 0.4753   0.4906
Analysis of Deviance Table (Type II Wald chisquare tests)

Response: Percent_microbial
                    Chisq Df Pr(>Chisq)
Age_class          0.1303  1     0.7182
plot_age           2.6802  1     0.1016
Age_class:plot_age 0.4783  1     0.4892

```{r}
microbialAgeSG_lm <- glmmTMB(Percent_microbial~ Age_class*plot_age+(1|Plot/Tree_num), data = subset(damagePlots, Sample_period == 1 & Species == "Sweet gum"), family = genpois)

Anova(microbialAgeSG_lm)
```
non-integer counts in a genpois modelNA/NaN function evaluationAnalysis of Deviance Table (Type II Wald chisquare tests)

Response: Percent_microbial
                      Chisq Df Pr(>Chisq)    
Age_class          128.6568  2     <2e-16 ***
plot_age             2.3516  1     0.1252    
Age_class:plot_age   0.7251  2     0.6959    
---
Signif. codes:  0 ‘***’ 0.001 ‘**’ 0.01 ‘*’ 0.05 ‘.’ 0.1 ‘ ’ 1


```{r}
microbialAgeSG2_lm <- glmmTMB(Percent_microbial~ Age_class*plot_age+(1|Plot/Tree_num), data = subset(damagePlots, Sample_period == 2 & Species == "Sweet gum"), family = genpois)

Anova(microbialAgeSG2_lm)
```
non-integer counts in a genpois modelNA/NaN function evaluationAnalysis of Deviance Table (Type II Wald chisquare tests)

Response: Percent_microbial
                    Chisq Df Pr(>Chisq)  
Age_class          3.0034  1    0.08309 .
plot_age           0.7624  1    0.38259  
Age_class:plot_age 2.3202  1    0.12771  
---
Signif. codes:  0 ‘***’ 0.001 ‘**’ 0.01 ‘*’ 0.05 ‘.’ 0.1 ‘ ’ 1

```{r}
microbialAgeTP_lm <- glmmTMB(Percent_microbial~ Age_class*plot_age+(1|Plot/Tree_num), data = subset(damagePlots, Sample_period == 1 & Species == "Tulip poplar"), family = genpois)

Anova(microbialAgeTP_lm)
```
non-integer counts in a genpois modelNA/NaN function evaluationAnalysis of Deviance Table (Type II Wald chisquare tests)

Response: Percent_microbial
                    Chisq Df Pr(>Chisq)  
Age_class          8.8390  2    0.01204 *
plot_age           2.6985  1    0.10044  
Age_class:plot_age 4.1974  2    0.12262  
---
Signif. codes:  0 ‘***’ 0.001 ‘**’ 0.01 ‘*’ 0.05 ‘.’ 0.1 ‘ ’ 1

```{r}
microbialAgeTP2_lm <- glmmTMB(Percent_microbial~ Age_class*plot_age+(1|Plot/Tree_num), data = subset(damagePlots, Sample_period == 2 & Species == "Tulip poplar"), family = genpois)

Anova(microbialAgeTP2_lm)
```
...
Response: Percent_microbial
                    Chisq Df Pr(>Chisq)
Age_class          0.3022  1     0.5825
plot_age           2.4897  1     0.1146
Age_class:plot_age 0.7598  1     0.3834
...
ggplot(damagePlots, aes(x=plot_age, y=Percent_total_herbivory)) +
  geom_point(aes(color=Age_class)) + 
  scale_color_manual(values = c("yellow3",  "chartreuse4", "darkorchid3")) +
  geom_smooth(aes(color=Age_class, fill=Age_class), size=1, method=lm) +
  scale_x_continuous(breaks=number_ticks(4)) +
  facet_grid(rows = vars(Species), cols = vars(Sample_period), labeller = labeller(Sample_period = sp.labs))+
  theme(strip.text.x = element_text(size = 14, color = "black"), strip.text.y = element_text(size = 14, color = "black"))+
  ylab("Percent Herbivore Damage")+
  xlab("Plot Age")+
...
ggplot(damagePlots, aes(x=plot_age, y=Percent_microbial)) +
  geom_point(aes(color=Age_class)) + 
  scale_color_manual(values = c("yellow3",  "chartreuse4", "darkorchid3")) +
  geom_smooth(aes(color=Age_class, fill=Age_class), size=1, method=lm) +
  scale_x_continuous(breaks=number_ticks(4)) +
...
*one model for each herbivory type, by species, sample period and age class*

```{r}

herbChewingModel <- glmmTMB(Percent_chewed~ Species*Age_class*Sample_period+(1|Plot/Tree_num), data = damagePlots, family = genpois)

herbChewingModel_O <- glmmTMB(Percent_chewed~ Species*Age_class +(1|Plot/Tree_num), data = damageOct, family = genpois)



##model checking##
model_checkChew <- simulateResiduals(herbChewingModel)
plot(model_checkChew) 

model_checkChewO <- simulateResiduals(herbChewingModel_O)
plot(model_checkChewO) 


#Amy also ran...#
testZeroInflation(herbChewingModel)
testUniformity(herbChewingModel) 
testOutliers(herbChewingModel) 

drop1(herbChewingModel, test = 'Chisq')

drop1(herbChewingModel_O, test = 'Chisq')

Anova(herbChewingModel)

Anova(herbChewingModel_O)

emmeans(herbChewingModel, pairwise~as.factor(Species)*as.factor(Age_class)*as.factor(Sample_period), adjust="tukey", type = "response")
```

```{r}
herbMiningModel <- glmmTMB(Percent_mined~ Species*Age_class*Sample_period+(1|Plot/Tree_num), data = damagePlots, family = genpois)

##model checking##
model_checkMined <- simulateResiduals(herbMiningModel)
plot(model_checkMined) 

#Amy also ran...#
testZeroInflation(herbMiningModel)
testUniformity(herbMiningModel) 
testOutliers(herbMiningModel) 

drop1(herbMiningModel, test = 'Chisq')


Anova(herbMiningModel)


emmeans(herbMiningModel, pairwise~as.factor(Species)*as.factor(Age_class)*as.factor(Sample_period) adjust="tukey", type = "response")
```

```{r}
herbSkeletonizingModel <- glmmTMB(Percent_skeletonized~ Species*Age_class*Sample_period+(1|Plot/Tree_num), data = damagePlots, family = genpois)

##model checking##
model_checkSkel <- simulateResiduals(herbSkeletonizingModel)
plot(model_checkSkel) 

#Amy also ran...#
testZeroInflation(herbSkeletonizingModel)
testUniformity(herbSkeletonizingModel) 
testOutliers(herbSkeletonizingModel) 

drop1(herbSkeletonizingModel, test = 'Chisq')


Anova(herbSkeletonizingModel)


emmeans(herbSkeletonizingModel, pairwise~as.factor(Species)*as.factor(Age_class)*as.factor(Sample_period) adjust="tukey", type = "response")
```

```{r}
herbGallModel <- glmmTMB(Percent_gall~ Species*Age_class*Sample_period+(1|Plot/Tree_num), data = damagePlots, family = genpois)

##model checking##
model_checkGall <- simulateResiduals(herbGallModel)
plot(model_checkGall) 

#Amy also ran...#
testZeroInflation(herbGallModel)
testUniformity(herbGallModel) 
testOutliers(herbGallModel) 

drop1(herbGallModel, test = 'Chisq')


Anova(herbGallModel)


emmeans(herbGallModel, pairwise~as.factor(Species)*as.factor(Age_class)*as.factor(Sample_period) adjust="tukey", type = "response")
```



```{r}
herbivorywide <- damagePlots %>%
  select (Plot, Species, Age_class, Leaf_num, Sample_period, Percent_chewed, Percent_mined, Percent_skeletonized, Percent_gall, plot_age, plot_age_class, Tree_num)
...
herbivorylongSeedling <- herbivorylong %>%
  filter(Age_class == "Seedling")

ggplot(data=barGraphStats(data=herbivorylongSeedling, variable="Percent_damage", byFactorNames=c("Species", "Sample_period", "Herbivory_type")), aes(x=Sample_period, y=mean, fill=Herbivory_type))+
  geom_bar(position="stack", stat="identity")+
  scale_fill_discrete(labels = c("Chewing", "Mining", "Skeletonizing", "Galls"))+
  facet_wrap(~Species)+
  theme(strip.text.x = element_text(size = 14, color = "black"))+
...
herbTypeModel <- glmmTMB(Percent_damage~ Species*Age_class*Sample_period*Herbivory_type+(1|Plot/Tree_num), data = subset(herbivorylong, Age_class != "Seedling"), family = genpois)
...
##model checking##
model_checkHerb <- simulateResiduals(herbTypeModel)
plot(model_checkHerb) 


#Amy also ran...#
testZeroInflation(herbTypeModel)
testUniformity(herbTypeModel) 
testOutliers(herbTypeModel) 

drop1(herbTypeModel, test = 'Chisq')
...
emmeans(herbTypeModel, pairwise~as.factor(Species)*as.factor(Age_class)*as.factor(Sample_period)*as.factor(Herbivory_type), adjust="tukey", type = "response")
...
Show in New Window
The working directory was changed to C:/Users/hrusk/Dropbox (Smithsonian)/SERC_damageDEMO inside a notebook chunk. The working directory will be reset when the chunk is finished running. Use the knitr root.dir option in the setup chunk to change the working directory for notebook chunks.
Show in New Window
Analysis of Deviance Table (Type II Wald chisquare tests)

Response: Percent_chewed
                     Chisq Df Pr(>Chisq)    
Species           161.1786  2     <2e-16 ***
Age_class           0.2950  1     0.5870    
Species:Age_class   2.7636  2     0.2511    
---
Signif. codes:  0 ‘***’ 0.001 ‘**’ 0.01 ‘*’ 0.05 ‘.’ 0.1 ‘ ’ 1
Show in New Window
Description:df [24,968 x 10]
Plot
<chr>
Species
<fctr>
...
plot_age_class
<int>
CI1	Sweet gum	Mature	CI1SGMat4L5	1	80	3	
CI1	Beech	Mature	CI1BMat3L5	1	80	3	
CI1	Sweet gum	Mature	CI1SGMat4L4	1	80	3	
CI1	Beech	Mature	CI1BMat2L19	2	80	3	
CI1	Tulip poplar	Mature	CI1TPMat5L8	1	80	3	
CI1	Beech	Mature	CI1BMat3L4	1	80	3	
CI1	Beech	Mature	CI1BMat3L1	1	80	3	
CI1	Beech	Mature	CI1BMat2L6	1	80	3	
CI1	Sweet gum	Mature	CI1SGMat4L6	1	80	3	
CI1	Sweet gum	Mature	CI1SGMat3L11	2	80	3	
...
1-10 of 24,968 rows | 1-7 of 10 columns
Show in New Window
Analysis of Deviance Table (Type II Wald chisquare tests)

Response: Percent_damage
                                                   Chisq Df Pr(>Chisq)    
Species                                         804.9993 23  < 2.2e-16 ***
Age_class                                        15.2777 15    0.43161    
Sample_period                                   203.6171 19  < 2.2e-16 ***
Herbivory_type                                 1292.8602 25  < 2.2e-16 ***
Species:Age_class                                14.8949  7    0.03737 *  
Species:Sample_period                           120.0691 11  < 2.2e-16 ***
Age_class:Sample_period                          12.1838  7    0.09467 .  
Species:Herbivory_type                          678.3359 14  < 2.2e-16 ***
Age_class:Herbivory_type                          9.3515  8    0.31352    
Sample_period:Herbivory_type                    129.7756 11  < 2.2e-16 ***
Species:Age_class:Sample_period                   3.8585  3    0.27716    
Species:Age_class:Herbivory_type                 10.4548  7    0.16423    
Species:Sample_period:Herbivory_type             39.7197  7  1.424e-06 ***
Age_class:Sample_period:Herbivory_type            5.4933  4    0.24032    
Species:Age_class:Sample_period:Herbivory_type    4.2213  6    0.64676    
---
Signif. codes:  0 ‘***’ 0.001 ‘**’ 0.01 ‘*’ 0.05 ‘.’ 0.1 ‘ ’ 1
...
add 0.001 to all heribovry + microbial damage values to use log10 in graph
```{r}
damageRegression <- damagePlots %>%
  mutate(TotalHerbRegression = Percent_total_herbivory + 0.001)%>%
  mutate(TotalMicrobialRegression = Percent_microbial + 0.001)%>%
  mutate(logTotalHerbRegression = log10(TotalHerbRegression))%>%
  mutate(logTotalMicrobialRegression = log10(TotalMicrobialRegression))

damageRegression$Age_class <- factor(damageRegression$Age_class, levels = c("Seedling", "Sapling", "Mature"))
```
